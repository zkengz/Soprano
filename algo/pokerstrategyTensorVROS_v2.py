...
class VROutcomeSamplingSolver(object):
    ...
    def ev_helper(self, root, reachprobs, sampleprobs, path_policies):
        if type(root) is TerminalNode:
            return self.ev_terminal_node(root)
        if type(root) is HolecardChanceNode:
            return self.ev_holecard_node(root, reachprobs, sampleprobs, path_policies)
        if type(root) is BoardcardChanceNode:
            return self.ev_boardcard_node(root, reachprobs, sampleprobs, path_policies)
        return self.ev_action_node(root, reachprobs, sampleprobs, path_policies)
    ...
    def ev_holecard_node(self, root, reachprobs, sampleprobs, path_policies):
        assert(len(root.children) == 1)
        return self.ev_helper(root.children[0], reachprobs, sampleprobs, path_policies)
        
    def ev_boardcard_node(self, root, reachprobs, sampleprobs, path_policies):
        ...
        for bc in root.children:
            if self.boardmatch(num_dealt, bc):
                results = self.ev_helper(bc, reachprobs, sampleprobs, path_policies)
                return results
    ...
    def ev_action_node(self, root, reachprobs, sampleprobs, path_policies):
        ...
        csp = self.exploration * (1.0 / len(root.children)) + (1.0 - self.exploration) * action_probs[action] # 是否要传播梯度
        
        next_reachprobs = [rp.clone() for rp in reachprobs] # clone可以传播梯度
        next_reachprobs[root.player] *= action_probs[action]
        next_path_policies = {**path_policies, infoset: (action_probs, valid)} # 只是增加一个键值对，策略在本次迭代中不会变，不需要深拷贝
        payoffs = self.ev_helper(root.get_child(action), next_reachprobs, sampleprobs * csp, next_path_policies) # 此处payoff是u(h,a)
        ...
        if self.baseline_type == "na": # MCCFR
            for player in range(self.rules.players):
                ...
                u_act_bl[action] += payoffs[player] / csp.detach() # TODO：需要detach，因为u_act_bl只是价值估计，不传播梯度？
                payoffs[player] = torch.dot(u_act_bl, action_probs.detach()) # u(h) # 递归往上传 # TODO:是否要detach？似乎无所谓，这部分参与计算上层的反事实价值，但上层不知道下层采取什么策略，只会更新到上层为止的行为策略
                ...
                if player == root.player:
                    ...
                    cfv = torch.dot(cfq, action_probs) # v(I)
                    # FOR DEBUG
                    if self.debug:
                ...
                if player == root.player:
                    ...
                    for opp in range(self.rules.players):
                        if opp != root.player:
                            opp_rp = opp_rp * reachprobs[opp]
                    cfq = u_act_bl * opp_rp / sampleprobs # v(I,a) (or v(h,a)) for all action a
                    cfv = torch.dot(cfq, action_probs) # v(I) # cfq仅估计值，不传播梯度
                    # FOR DEBUG
                    if self.debug:
                        self.cfv[infoset] += cfv
                        self.cfv_cnt[infoset] += 1
        ...
        # 对历史的所有行为策略，计算反事实价值对动作的梯度
        for path_infoset, (path_policy, path_valid) in path_policies.items():

            grad = torch.autograd.grad(cfv, path_policy, create_graph=True)[0]
            
            # 计算动作熵的梯度
            valid_ent_grad = - torch.log(torch.clamp(torch.masked_select(path_policy, path_valid), 1e-5, 1)) - 1 # clamp避免log(0)
            ent_grad = torch.zeros_like(grad)
            ent_grad.masked_scatter_(path_valid, valid_ent_grad)
            # 计算总梯度的投影的范数，作为损失函数
            proj_grad = projected_gradient(path_valid, grad + self.tao * ent_grad)
            
            # self.profile.proj_grad_norms[root.player][infoset] = proj_grad_norm

            old_proj_grad = self.proj_grad_dict.get(path_infoset)
            if old_proj_grad is not None:
                self.norm_dict[path_infoset] = self.norm_dict[path_infoset] + torch.dot(proj_grad, old_proj_grad)
                del self.proj_grad_dict[path_infoset]
                self.norm_cnt[path_infoset] += 1
            else:
                self.proj_grad_dict[path_infoset] = proj_grad
