...
class CounterfactualRegretMinimizer(object):
    ...
    def cfr_action_node(self, root, reachprobs):
        ...
        strategy = self.cfr_strategy_update(root, reachprobs) # 这里是用当前的reachprobs传入计算的
    ...
    def cfr_strategy_update(self, root, reachprobs):
        ...
        for hc in root.holecards[root.player]:  
            ...
            for i in range(3):
                self.action_reachprobs[root.player][infoset][i] += reachprobs[root.player][hc] * probs[i]
            self.profile.strategies[root.player].policy[infoset] = [self.action_reachprobs[root.player][infoset][i] / sum(self.action_reachprobs[root.player][infoset]) for i in range(3)] # policy normalization
...
class ChanceSamplingCFR(CounterfactualRegretMinimizer):
    """
    只采样发牌，但会遍历每个动作（相当于每个信息集取一个节点，博弈树的“形状”保留）
    """
    def __init__(self, rules):
        CounterfactualRegretMinimizer.__init__(self, rules)
...
class OutcomeSamplingCFR(ChanceSamplingCFR):
    """
    只采样一条轨迹
    sampleprobs对应叶节点的采样概率q(z)，用于计算采样反事实价值
    为了保证探索，还引入探索率exploration，以该概率随机采动作，而非按策略采动作
    """
    ...
    def cfr_regret_update(self, root, ev, action, actionprob):
        ...
        for i in range(3):
            if not root.valid(i):
                continue
            immediate_cfr = -ev * actionprob # -v^c(I)
            if action == i:
                immediate_cfr += ev # q^c(I,a)
            self.counterfactual_regret[root.player][infoset][i] += immediate_cfr
