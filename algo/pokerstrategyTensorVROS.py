...
class StrategyTensor(object):
    ...
    def sample_action(self, infoset):
        ...
        #         return i
        # raise Exception('Invalid probability distribution. Infoset: {0} Probs: {1}, val:{2}, total:{3}'.format(infoset, probs ,val, total))

        probs = self.policy[infoset].detach()
...
class VROutcomeSamplingSolver(object):
    def __init__(self, rules, teams, exploration=0.1, lr=0.01, tao=0.01, sample_traj_num=10, 
                 ...
                 save_path=None, seed=0, debug=False):
        self.rules = rules
        ...
        for s in self.profile.strategies:
            if seed == None:
                ...
                s.build_random(self.tree, seed=seed)
        ...
        # self.optimizer = torch.optim.SGD(list(self.params.values()), lr=lr, momentum=0.9)
        # self.optimizer = torch.optim.SGD(list(self.params.values()), lr=lr, momentum=0.9, nesterov=True)
    ...
    def run(self, num_iterations, eval_freq=100):
        for iteration in range(num_iterations):
            if iteration % eval_freq == 0:
                ...
                if self.debug:
                    print(f"loss: {self.loss}, grad norm:{calculate_grad_norm(self.optimizer.param_groups[0]['params'])}")
    ...
    def update(self):
        ...
        self.loss = sum(self.norm_dict.values())
    ...
    def ev_action_node(self, root, reachprobs, sampleprobs):
        ...
        if self.baseline_type == "na": # MCCFR
            for player in range(self.rules.players):
                ...
                payoffs[player] = torch.dot(u_act_bl, action_probs) # u(h) # 递归往上传 # TODO:是否要detach？不detach相当于上层的损失能传到下层策略
        ...
        ent_grad = torch.zeros_like(cfq)
        ent_grad.masked_scatter_(valid, valid_ent_grad)
        # 计算总梯度的投影的范数，作为损失函数
        proj_grad = projected_gradient(valid, cfq + self.tao * ent_grad)
